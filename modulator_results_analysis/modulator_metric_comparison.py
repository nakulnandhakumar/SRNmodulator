...
df_HfO2_ALD_shield = pd.read_csv("modulator_sweep_data/HfO2_ALDshield.csv")
df_HfO2_NonALD_shield = pd.read_csv("modulator_sweep_data/HfO2_NonALDshield.csv")
df_SiO2_shield = pd.read_csv("modulator_sweep_data/SiO2_shield.csv")


# Convert gap from meters to nanometers for better readability
...
# Plot loss vs gap for just the HfO2 non-ALD shield configuration
plt.figure(figsize=(8, 6))
plt.plot(df_HfO2_NonALD_shield['Gap'], df_HfO2_NonALD_shield['Loss'], marker='s', label='HfO2 Shield (Top Layer Etched)', color='green')
plt.xlabel('Gap (nm)')
plt.ylabel('Loss (dB/cm)')
plt.title('Loss vs Gap (HfO2 Shield (Top Layer Etched))')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
